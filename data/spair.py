...
class SPairDataset(CorrespondenceDataset):
    ...
    def __init__(self, benchmark, datapath, thres, device, split, augmentation, feature_size=64, receptive_field_size=11):
        ...
        super(SPairDataset, self).__init__(benchmark, datapath, thres, device, split, augmentation, feature_size,receptive_field_size)
        ...
        anntn_files = []
        for data_name in self.train_data: #TODO fix this
            search_name = os.path.join(self.ann_path, data_name+'.json')
            #print('debug search name {}'.format(search_name))

            if os.path.exists(search_name):
                anntn_files.append(glob.glob('%s/%s.json' % (self.ann_path, data_name))[0])
            else:
                new_name = '-'.join(data_name.split(':'))
                anntn_files.append(glob.glob('%s/%s.json' % (self.ann_path, new_name))[0])
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'trn' and self.augmentation:
            batch['src_img'], batch['src_kps'] = random_crop(batch['src_img'], batch['src_kps'], self.src_bbox[idx].clone(), size=(self.imside,)*2,p=-1.0)
            batch['trg_img'], batch['trg_kps'] = random_crop(batch['trg_img'], batch['trg_kps'], self.trg_bbox[idx].clone(), size=(self.imside,)*2,p=-1.0)
