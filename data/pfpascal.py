...
class PFPascalDataset(CorrespondenceDataset):
    def __init__(self, benchmark, datapath, thres, device, split, augmentation, feature_size=64, receptive_field_size=11):
        #r"""PF-PASCAL dataset constructor"""
        #print('verbose...PFPascalDatasetAugv0dot2Nocrop rfz {}'.format(receptive_field_size))
        #super(PFPascalDatasetAugv0dot2Nocrop, self).__init__(benchmark, datapath, thres, device, split, augmentation, feature_size,receptive_field_size)
        r"""PF-PASCAL dataset constructor"""
        super(PFPascalDataset, self).__init__(benchmark, datapath, thres, device, split, augmentation, feature_size,receptive_field_size)
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'trn' and self.augmentation:
            batch['src_img'], batch['src_kps'] = random_crop(batch['src_img'], batch['src_kps'], self.src_bbox[idx].clone().int(), size=(self.imside,)*2, p=-1.0) # always resize, no crop
            batch['trg_img'], batch['trg_kps'] = random_crop(batch['trg_img'], batch['trg_kps'], self.trg_bbox[idx].clone().int(), size=(self.imside,)*2, p=-1.0) # always resize, no crop

        # Object bounding-box (resized following self.imside) used for computing pck@bbox
